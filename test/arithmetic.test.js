...
describe('Power', function () {
    ...
    it('rejects invalid power operation', function (done) {
        request.get('/arithmetic?operation=power&operand1=2&operand2=foobar')
            .expect(400)
            .end(function (err, res) {
                expect(res.body).to.eql({ error: "Invalid operand2: foobar" });
                done();
            });
    });
});
